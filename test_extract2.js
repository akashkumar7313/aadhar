...
function _isGarbled(t) {
  ...
  var bad = (clean.match(/[\uFFFD\u25A0-\u25FF\u2580-\u259F□■◻◼▪▫\uE000-\uF8FF\uF0000-\uFFFFF]/g) || []).length;
...
function parseOCR(text, state) {
  ...
  for (var ni = 0; ni < lines.length; ni++) {
    ...
    var score = 0;
    if (dobIdx > -1 && ni < dobIdx && ni >= dobIdx - 4) score += 10;
  ...
  for (var ei = 0; ei < lines.length; ei++) {
    ...
    if (dobIdx > -1 && ei < dobIdx && ei >= dobIdx - 5) escore += 10;
  ...
  var hindiAddr = lines.filter(function (l) {
    ...
    return (cl.indexOf(',') > -1 || cl.indexOf('-') > -1 || /\d{6}/.test(cl) ||
      /ग्राम|पोस्ट|जिला|नगर|राज्य|प्रदेश|मोहल्ला|मकान|वार्ड|पता|अर्धांगिनी|पत्नी|पुत्र|पुत्री|पोस्ट|डाकघर|एस\/ओ|सी\/ओ|डब्ल्यू\/ओ/.test(cl));
  }).map(_cleanLine);
    ...
    try {
      ...
      for (let i = 1; i <= Math.min(1, pdf.numPages); i++) {
        ...
        tc.items.forEach(function (it) {
          ...
          rawItems.push({ s: s, x: tx[4], y: tx[5] - th, w: tw, h: th, rawY: it.transform[5] || 0 });
        });

        /* simulate crop to CARD AREA: user crops the "niche ke hisse" = lower card portion */
        const CROP_Y_MIN = 1100;
        const CROP_Y_MAX = 1500;
        let picked = rawItems.filter(it =>
          it.y + it.h >= CROP_Y_MIN && it.y <= CROP_Y_MAX
        );

        /* Sort + X-gap column splitting = same logic as updated extractPdfTextInRect */
        picked.sort(function (a, b) { var dy = a.y - b.y; return Math.abs(dy) <= 2 ? a.x - b.x : dy; });
        const out = []; let lastLy = null; let lastLxEnd = null;
        const gapEst = Math.max(100, vp.width * 0.08);
        for (const p of picked) {
          if (lastLy !== null) {
            if (Math.abs(p.y - lastLy) > 4) {
              out.push('\n');
            } else if (lastLxEnd != null && p.x - lastLxEnd > gapEst) {
              out.push('\n');
            }
          }
          out.push(p.s);
          lastLy = p.y;
          const e = p.x + Math.max(p.w || 4, (p.s || '').length * 5);
          if (lastLxEnd == null || e > lastLxEnd) lastLxEnd = e;
        }
